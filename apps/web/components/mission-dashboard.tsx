...
export function MissionDashboard() {
  ...
  const eventStops = useMemo(() => bundle.events.map((event) => ({
    ...event,
    index: bundle.trajectory.samples.findIndex((sample) => sample.timestamp >= event.timestamp)
    ...
    <main className="mx-auto flex min-h-screen max-w-7xl flex-col gap-6 px-6 py-8">
      <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-slate-300">Phase 2 makes the viewer more honest and more alive: Earth/Moon geometry is now tracked through a SPICE-first kernel manifest plus offline sampled Moon vectors, while Artemis II remains the flagship mission with a documented hybrid Orion path.</p>
        </div>
        <div className="rounded-2xl border border-cyan-400/30 bg-cyan-400/10 px-4 py-3 text-sm text-cyan-100">
          ...
          <div>Frame: <strong>{bundle.spice.frame}</strong></div>
        </div>
      </header>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.6fr_1fr]">
          ...
          <Card title="Timeline + controls" eyebrow="Mission control">
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-slate-950/40 p-3 text-sm text-slate-300">
                <div><strong>Current frame time:</strong> {new Date(sample.timestamp).toUTCString()}</div>
                <div><strong>Scene linkage:</strong> Moon position, Earth-Moon spacing, and camera targets follow the active sample instead of a fixed placeholder Moon.</div>
              </div>
              <div className="grid gap-2 lg:grid-cols-2">
                {bundle.events.map((event) => (
                  <button key={event.id} className={`rounded-xl border p-3 text-left ${event.timestamp === telemetry.nextEvent?.timestamp ? 'border-cyan-300 bg-cyan-300/10' : 'border-white/10 bg-white/5'}`} onClick={() => {
                    const stop = eventStops.find((entry) => entry.id === event.id);
                    if (stop) {
                      setPlaybackMode('replay');
                      setSampleIndex(stop.index);
                    }
                  }}>
        ...
        <div className="space-y-6">
          ...
          <Card title="SPICE kernel contract" eyebrow="Offline ingest boundary">
            <div className="space-y-3 text-sm text-slate-300">
              <p>{bundle.spice.notes[0]}</p>
              <ul className="space-y-2">
                {bundle.spice.kernels.map((kernel) => (
                  <li key={kernel.id} className="rounded-xl border border-white/10 bg-white/5 p-3">
                    <div className="font-medium text-white">{kernel.id} · {kernel.type}</div>
                    <div>{kernel.purpose}</div>
                  </li>
                ))}
              </ul>
            </div>
